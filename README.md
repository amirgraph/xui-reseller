...
| ✖️ **ضریبِ مصرف** | نماینده (اگر ادمین اجازه دهد) روی هر کانفیگ ضریب می‌گذارد — مثلاً ۲× = هر گیگِ واقعی، دو گیگ حساب می‌شود |
...
نصب‌کننده می‌پرسد: **CDN** (Cloudflare / آروان / هر دو)، دامنه‌ها، توکنِ ربات و ادمین، یوزر/رمزِ x-ui، قیمت‌گذاری، IP سرور. کلیدهای امنیتی (JWT و …) خودکار ساخته می‌شوند. اگر **آروان** را بزنی، اینباندِ `ws` + locationِ nginx + سرورِ آروان خودکار ساخته می‌شوند؛ اگر **فقط Cloudflare**، هیچ‌کدام. (بعضی فقط آروان می‌خواهند.)
...
### 🔐 دسترسی به پنل‌ها (آدرس‌های ورود)

| پنل | آدرس | ورود |
|---|---|---|
| **پنلِ ادمین** | `https://<دامنهٔ اصلی>/admin` | یوزر/رمزِ ادمین (هنگام نصب) |
| **پنلِ نماینده** | `https://<دامنهٔ اصلی>/panel` | یوزر/رمزِ هر نماینده |
| **کانفیگ‌های فروخته‌شدهٔ ربات** | همان `…/panel` | با یوزرِ **`direct_sales`** (رمز در پنلِ ادمین → نمایندگان) |

- **ادمین** همه‌چیز را می‌بیند و مدیریت می‌کند (نمایندگان، پلن‌ها، سرورها، تنظیمات، درخواست‌های خرید).
- **نماینده** فقط کاربران/کیف‌پول/ساب‌های خودش را می‌بیند.
- کانفیگ‌هایی که **ربات مستقیم می‌فروشد** زیرِ نمایندهٔ ویژهٔ `direct_sales` (مالِ ادمین) جمع می‌شوند → از همان پنلِ نماینده مدیریت می‌شوند.
- ریشهٔ دامنه (`/`) خودکار به `/panel` هدایت می‌شود.
